...
*/
class tx_vdexcavationexchange_offerlot extends tslib_pibase {
	var $prefixId = 'tx_vdexcavationexchange_offerlot';		// Same as class name
	var $scriptRelPath = 'pi1/offerlot/class.tx_vdexcavationexchange_offerlot.php';	// Path to this script relative to the extension dir.
	var $extKey = 'vd_excavationexchange';	// The extension key.
	//
	var $folderRelPath; // path to the current folder
	var $table = 'tx_vdexcavationexchange_offerlot';
	...
	*/
	function ini($conf)	{
		...
		// config
		$this->folderRelPath = $GLOBALS['TYPO3_LOADED_EXT'][$this->extKey]['siteRelPath'].'/pi1/offerlot';
		$this->setFieldsItems();
	}
	...
	function select($uid, $options){
		...
		$q_limit = $options['limit'];
		$q_fields =  $options['fields']? $options['fields']:'*';
		$qw_where_in = $options['where_in'];
    ...
    // access check 
		...
		// where in  
		$q_where = ($q_where && $qw_where_in)?$q_where.' AND '.$qw_where_in:$q_where.$qw_where_in;
		
		// $qw_access has the AND already
		$q_where = ($q_where && $qw_access)?$q_where.' '.$qw_access:$q_where.$qw_access;
		// orderBy
		$q_orderBy = $options[orderBy]<>''?$options[orderBy]:''; 
		$res=$GLOBALS['TYPO3_DB']->exec_SELECTquery($q_fields,$this->table,$q_where,'',$q_orderBy,$q_limit);                    
		$record = $this->fetchQuery($res);
		// transform records for lists and date
		$this->fieldsItems;
		$list = array('catmatterrial','designiationuscsmeuble','designiationsimplemeuble','designiationsimplemeubleha','designiationsimplemeublehb','designiationroche','designiationrocheautre');
	...
	*/
	...
	function update($data, $options=''){
		...
		// do not update the offer id ref
		unset($fields_values['offer_id']);
...
*/
	function setFieldsItems(){
		...
		t3lib_div::loadTCA('tx_vdexcavationexchange_offerlot');
		$tableconf = $TCA['tx_vdexcavationexchange_offerlot'];
	...
	function get_recordsArchiveCount(){
		if($this->recordsArchiveCount){
			...
			$qw_archive = $this->table.'.archive = 1';    
			// access check
			$qw_access = t3lib_BEfunc::deleteClause($this->table);
			// $res = t3lib_BEfunc::getRecordsByField($this->table,'archive',0, $qw_access);
			$res=$GLOBALS['TYPO3_DB']->exec_SELECTquery("count(uid)",$this->table,$qw_pid.' AND '.$qw_archive .' '. $qw_access);  
			$record = $this->fetchQuery($res);
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/vd_excavationexchange/pi1/offerlot/class.tx_vdexcavationexchange_offerlot.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/vd_excavationexchange/pi1/offerlot/class.tx_vdexcavationexchange_offerlot.php']);
}
